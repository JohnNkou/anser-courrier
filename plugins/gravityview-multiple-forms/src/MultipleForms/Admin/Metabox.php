...
class Metabox extends AbstractSingleton {
	...
	public function add_setting( $current_settings ): void {
		...
		// These settings are updated on new Views only to avoid breaking changes.
		$is_new_view = 0 === (int) ( $_GET['post'] ?? null );

		if ( $is_new_view ) {
			$current_settings['show_only_approved_joined'] = 1;
		}

		GravityView_Render_Settings::render_setting_row( 'show_only_approved_joined', $current_settings );
	...
	public function add_default_view_setting( $defaults = [] ): array {
		$defaults['show_only_approved_joined'] =
			[
				'label'             => 'Show only approved joined entries',
				'type'              => 'checkbox',
				'group'             => 'default',
				'desc'              => 'By default, only approved joined entries are displayed in a View. When enabled, this setting prevents unapproved or disapproved joined entries from appearing in results. If disabled, joined entries with all approval statuses will be visible, including disapproved entries.',
				'tooltip'           => false,
				'value'             => 0, // Default is 0, but is overwritten to `1` for new Views.
