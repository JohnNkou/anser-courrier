...
class Query extends GF_Query {
	...
	/**
	 * Prepares the order column.
	 *
	 * This method will make sure numeric order are sorted as such.
	 *
	 * @since 0.4.0
	 *
	 * @param string|int $sort_field          The field to sort by.
	 * @param string|int $form_id             The form ID.
	 * @param bool|null  $force_order_numeric Whether to force a numeric sort.
	 *
	 * @return GF_Query_Column|GF_Query_Call The sort column.
	 */
	private function prepare_order( $sort_field, $form_id, ?bool $force_order_numeric = null ) {
		$order = Sort::instance()->configure_order_column( $sort_field, $form_id );

		if (
			$force_order_numeric === null
			&& ! $order->is_entry_column()
		) {
			$sort_field = $order->field_id;
			$form_id    = $order->source;
	...
	public function parse( $form_id, $search_criteria = [], $sorting = null, $paging = null ) {
		 ...
		 */
		$is_numeric = $sorting['is_numeric'] ?? null;
		if ( $is_numeric !== null ) {
			$is_numeric = (bool) $is_numeric;
		}

		$order = $this->prepare_order( $sort_field, $form_id, $is_numeric );
	...
	public function order( $column, $order = null ) {
		...
		if ( $column instanceof GF_Query_Call ) {
			...
			if ( ! in_array( $order, [ self::ASC, self::DESC ], false ) ) {
				$order = self::ASC;
			}

			$order_column = $this->prepare_order( $column->field_id, $column->source );
			// If the column is cast as numeric, we need to retrieve the column from that call.
			$column = $order_column;
			if ( $order_column instanceof GF_Query_Call ) {
				$columns = $order_column->columns;
			...
			$this->order["{$source_id}_{$field_id}"] = [ $order_column, $order ];
		}
	...
	private function query() {
		 ...
		 */
		$sql = apply_filters( 'gform_gf_query_sql', compact( 'select', 'from', 'join', 'where', 'order', 'paginate' ), $this );
		$sql = implode( ' ', array_filter( $sql, 'strlen' ) );

		GFCommon::log_debug( __METHOD__ . '(): sql => ' . $sql );

		$this->timer_start();
		$results          = $wpdb->get_results( $sql, ARRAY_N ); flogs("SQL IS %s",$sql);
	 ...
	 * @internal
	 *
	 * @param GF_Query_Condition $condition A condition (from self::$where)
	 *
	 * @return string[] The individual JOIN statements.
	 */
	...
	public function _prime_joins( $explicit_joins, $where_inference_joins = null, $order_inference_joins = null ) {
		...
		$explicit_join_aliases = apply_filters(
			'gk/multiple-forms/query/explicit-join-aliases',
			[],
			$this,
			compact( 'explicit_joins', 'where_inference_joins', 'order_inference_joins' )
		);
		...
		foreach ( array_merge( $where_inference_joins, $order_inference_joins ) as $join ) {
			if ( preg_match( '#` AS `([motc]\d+)` ON #', $join, $matches ) ) {
				if ( ! in_array( $matches[1], $explicit_join_aliases ) ) {
					// Get form ID and field ID from alias.
					if ( ( $key = array_search( $matches[1], $this->aliases, true ) ) !== false ) {
						[ $form_id, $field_id ] = explode( '_', $key );
						// In case of an explicit join on a Meta, Order or Cast key.
						if ( isset( $explicit_join_aliases[ $form_id ] ) && strpos( $explicit_join_aliases[ $form_id ], 't' ) !== 0 ) {
							[ $table, $on ] = explode( ' ON ', $join );
							// Replace the default entry ID JOIN with one that references the correct table.
							$on   = preg_replace(
								sprintf( '#`%s`.`entry_id` = `(?:[motc]\d+)`.`(?:.*?)`#', $matches[1] ),
								sprintf( '`%s`.`entry_id` = `%s`.`entry_id`', $matches[1], $explicit_join_aliases[ $form_id ] ),
								$on
							);
							$join = implode( ' ON ', [ $table, $on ] );
						}
					}
