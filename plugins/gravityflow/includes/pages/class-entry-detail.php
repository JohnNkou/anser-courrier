...
class Gravity_Flow_Entry_Detail {
	...
	public static function entry_detail( $form, $entry, $current_step = null, $args = array() ) {
		...
		if ( is_admin() ) {
			Locking\Locking::get_instance()->lock_info( $entry['id'], false );
		}
			...
			?>
				<form id="gform_<?php echo absint( $form_id) ; ?>" method="post" enctype='multipart/form-data' action="<?php echo esc_url( $url ); ?>" data-formid="<?php echo absint( $form_id ); ?>">
	...
	public static function fields( $form, $entry, $display_empty_fields, $current_step, $format ) {
		...
		foreach ( $form['fields'] as &$field ) {
			/* @var GF_Field $field */
			error_log("Displaying field ".json_encode($field,true));
			...
			switch ( RGFormsModel::get_input_type( $field ) ) {
				...
				case 'html':
					if ( $display_field ) {
						error_log("HTML FIELD ".print_r($field));
